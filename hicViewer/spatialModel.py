...
class spatialModel(object):
    ...
    def __init__(self):
        ...
        self.colors = [plt.cm.YlOrRd, plt.cm.YlOrBr, plt.cm.YlGnBu, plt.cm.YlGn, plt.cm.Reds, plt.cm.RdPu,
                       plt.cm.Purples,
                       plt.cm.PuRd, plt.cm.PuBuGn, plt.cm.PuBu, plt.cm.OrRd, plt.cm.Oranges, plt.cm.Greys,
                       plt.cm.Greens,
    ...
    def aggregateMatrix(self, mat5C, col):
        ...
        :rtype: object
        """
        order = np.lexsort(mat5C[:, :col].T)
        mat5C = mat5C[order]

        # get the unique row ids
        diff = np.ones(len(mat5C), 'bool')
        diff[1:] = (mat5C[1:,:col] != mat5C[:-1,:col]).any(-1)

        # sum their interaction frequency
        agg_freq = np.bincount(diff.cumsum() - 1, mat5C[:, col])

        ## return the aggregated method
        mat5C = mat5C[diff]
        mat5C[:, 4] = agg_freq
        return mat5C
    ...
    def prepareFolder(self, folder, resolution, alpha, beta, seed, chrs, HiC, method,normalize):
        ...
        try:
            with open(configFile, 'w') as cfgFile:
                ...
                ## convert it into a matrix, then save it
                ...
                if (hiCnp.shape[0] == hiCnp.shape[1]):
                    ...
                    if hiCnp.shape[1] == 3:
                        # aggregate similar interactions if any
                        hiCnp = self.aggregateMatrix(hiCnp,2)
                        ...
                        lengths1 = [lengths[:int(x-1)].sum() for x in hiCnp[:, 0]]
                        hiCnp[:, 1] = np.ceil(hiCnp[:, 1] / resolution)
                        hiCnp[:, 1] = hiCnp[:, 1] + lengths1

                        ## write the bin id for the left interactions
                        lengths2 = [lengths[:int(x-1)].sum() for x in hiCnp[:, 2]]
                        hiCnp[:, 3] = np.ceil(hiCnp[:, 3] / resolution)
                        hiCnp[:, 3] = hiCnp[:, 3] + lengths2

                        # aggregate similar interactions if any
                        hiCnp = self.aggregateMatrix(hiCnp,4)

                        dim = hiCnp[:, (1, 3)].max().astype(int) + 1
