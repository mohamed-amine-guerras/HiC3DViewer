...
$(document).ready(function () {
    ...
    var minLineColor = new THREE.Color(0xFFEDA0);
    var maxLineColor = new THREE.Color(0xFFF7FB);
    ...
    function displayInteractions(chr1, chr2, pos1, pos2, freq, maxFreq) {
        ...
        var highlightColor = maxLineColor.clone();
    ...
    *******************************************************/
    ...
    function getHicMap(genome, chromosome1, chromosome2){
        ...
        var url = "/displayHic/" + genome + '/' + chromosome1 + '/' + chromosome2
    ...
    function sequentialBuildChr() {
        ...
        for (var i in chromosomes) {
            ...
            binFacesIndex[i] = new Array(chromosomes[i].length);

            // Add the points
            for (var point = 0; point < chromosomes[i].length; point++) {

                var datum = chromosomes[i][point];
    ...
    function parallelBuildChr(){

        var dataPerWorker =1;
        // if there is less data then the default number of workers
        // then send one chromosome for in each worker
        var nbChr = Object.keys(chromosomes).length;
        if(nbChr < nbWorkers){
            nbWorkers = nbChr;
        }
        else{
            dataPerWorker = Math.ceil( nbChr / nbWorkers);
        }
        ...
        var chrs = Object.keys(chromosomes)
        var idx = 0;
        var workers;
        runingWorkers = 0
        ...
        for(var i =0; i < nbWorkers; i++){
            // get the id of the chromosomes to process
            var ChrIds = chrs.slice(idx, idx+ dataPerWorker);
            var toSend = pick(chromosomes, ChrIds);
            // prepare the web worker
            workers = new Worker("/static/js/worker.js");
            workers.onmessage = meshBuildingDone;
            //start processing
            workers.postMessage({chrs : toSend, ids: ChrIds , col: col});
            runingWorkers++;
            idx += dataPerWorker;
    ...
    function meshBuildingDone(e){


        runingWorkers--;
        if(e.type == "error"){
            alert("Error when displaying chromosomes in worker " + runingWorkers);
            return;
        }

        var loader = new THREE.JSONLoader();

        // update the min, max, and the sum values
        psum.add(new THREE.Vector3(e.data.psum.x, e.data.psum.y, e.data.psum.z));
        pmin.min(new THREE.Vector3(e.data.pmin.x, e.data.pmin.y, e.data.pmin.z));
		pmax.max(new THREE.Vector3(e.data.pmax.x, e.data.pmax.y, e.data.pmax.z));

        // add the created mesh to the screen
        for(var idx =0; idx < e.data.ids.length; idx++){
            var i = e.data.ids[idx];
            binFacesIndex[i] = e.data.binFacesIndex[i];

            // add the built mesh
            groups[i] = new THREE.Geometry();
            groups[i].chromosome = i;

            var geom = loader.parse(e.data.geometies[i]);
            ...
            var materials = [m];
            groups[i] = new THREE.Mesh(geom.geometry , new THREE.MeshFaceMaterial(materials));
            groups[i].matrixAutoUpdate = false;
            groups[i].updateMatrix();
        ...
        // if it is the last worker display every thing
        if(runingWorkers == 0) {
            setupScene();
        }
    };
    ...
    function setupScene() {
        ...
        getHicMap(actualGenome, 1, 1);

        $(document).on("regionSelected", getBrushedRegion);
        $(document).on("clearHic", function (e) {
            clearRegions()
        });
        ...
        $.getJSON(url, function (data) {
            if (Object.keys(data).length > 0) {
                if ("error" in data) {
                    ...
                    // if the browser does not support web workers, use the sequential version
                    if (typeof(Worker) == "undefined") {
                        sequentialBuildChr(data);
                    }
                    else {
                        //sequentialBuildChr(data)
                        parallelBuildChr(data);
                    }
                }
            }
        });
