/**
 * Created by wangmengjie on 15/6/18.
 */
...
$(document).ready(function () {
    ...
    var baseURL="/member/nadhir/HiC3DViewer";
    var lastchr1;
    var lastchr2;
    var chrwidth;
    ...
    function handleFiles(content, ishicdata) {
        ...
        if(ishicdata){
            ...
            for(var i = 0; i < chrStart.length; i ++){
    ...
    function initiateGUI() {
        ...
        colFolder.close();
        var linecolFolder = gui.addFolder("Contact lines color");
        linecolFolder.close();
        var otherParams = {
            'x': false,
            'y': false,
    ...
    function setupGUI(ishicdata) {
        ...
        if(ishicdata) {
            ...
            linecolFolder.add(linecol,'cis-lines width',0.1,5);
            ...
            linecolFolder.add(linecol,'trans-lines width',0.1,5);
            linecolFolder.close();
        }
        var otherParams = {
            'x': false,
        ...
        if(ishicdata) {
            otherFolder.add(otherParams, 'trans-interactions')
                .onChange(function (value) {
                    ...
                    else if (transLines && oldhighlightedRegions == highlightedRegions) {
    ...
    function showtranslines(value) {
        if(value) {
            ...
            oldhighlightedRegions = new Array(highlightedRegions);
        ...
        if(species) {
            ...
            if(species != "Others") {
                chrDrawZone.append("text")
                    ...
                    .attr("x", 0)
                    .attr("y", 12)
                    .text(species + " cytobands load successfully.")
            }
            else{
                chrDrawZone.append("text")
                    .style("font-family", "helvetica, arial, sans-serif")
                    ...
                    .attr("x", 0)
                    .attr("y", 12)
                    .text("Sorry, the cytobands of your species are not available.")
            }
            ...
            if(data) {
                ...
                if(data.centers) {
                    ...
                    for (var chr = 0; chr < chrStart.length - 1; chr++) {
                        chrLen = locus[chrStart[chr + 1] - 1];
                        var chrname = realchr[chr].replace(/chr/,'');
                        chromInfo[realchr[chr]] = { "chrLength": chrLen,
                            "bands": chrCytobands[chrname],
                            "chr": chrname};
                    }
                }
                    ...
                    if(data.hasOwnProperty('genome')){
                        if (data['genome'].length > 0) {
                            ...
                            for (i = 0; i < data['genome'].length; i++) {
                                $("#genomes").append($("<option></option>")
                                    .attr("value", data['genome'][i]['modelPath'] + '/' + data.result['genome'][i]['file'])
